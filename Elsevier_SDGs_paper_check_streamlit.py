...
def find_quoted_content(string):
    ...
    result_list = [match.group(1) for match in matches]
    return result_list
...
## for規則 引號 "" 中的內容沒有星號 *, 輸出你的文章中有出現在 SDGs 關鍵字的部分. 
def check_strings_0(article, substring_rule):
   #for substring_rule in article:
   #pattern = r'\b' + re.escape(substring_rule) + r'\w*'
   pattern = r'\b' + re.escape(substring_rule) + r'\b'
   matches_article = re.findall(pattern, article, flags=re.IGNORECASE)
   #matches_rule = []
   # if matches_article:
   #     #print(f"The substring '{substring}' starting in '{matches[0]}' is present in the article.")
   #     matches_rule.append(substring_rule)
   #else:
       #print(f"The substring '{substring}' is not found in the article.")
   return matches_article

## for規則 引號 "" 中的內容只有一個 星號 *, 輸出你的文章中有出現在 SDGs 關鍵字的部分.
def check_strings_1(article, substring_rule):
   #for substring_rule in article:
   #pattern = r'\b' + re.escape(substring_rule) + r'\w*'
   pattern = r'\b' + re.escape(substring_rule) + r'(?!\s)\w*'
   #pattern = r'\b' + re.escape(substring_rule) + r'\b'
   matches_article = re.findall(pattern, article, flags=re.IGNORECASE)
   #matches_rule = []
   # if matches_article:
...
## for規則 引號 "" 中的內容沒有星號 *, 當你的文章與此規則吻合, 輸出此規則. 
def check_strings_0_rule(article, substring_rule):
   #for substring_rule in article:
   #pattern = r'\b' + re.escape(substring_rule) + r'\w*'
   pattern = r'\b' + re.escape(substring_rule) + r'\b'
   matches_article = re.findall(pattern, article, flags=re.IGNORECASE)
...
# ## for規則 引號 "" 中的內容只有一個 星號 *, 當你的文章與此規則吻合, 輸出此規則.
# def check_strings_1_rule(article, substring_rule):
#     #for substring_rule in article:
#     #pattern = r'\b' + re.escape(substring_rule) + r'\w*'
#     pattern = r'\b' + re.escape(substring_rule) + r'(?!\s)\w*'
#     #pattern = r'\b' + re.escape(substring_rule) + r'\b'
...
def check_strings_1_rule(article, substring_rule):
    matches_rule = []
    # pattern_space = r"\s"
    # match_a = re.search(pattern_space, article)
    # match_s = re.search(pattern_space, substring_rule)
    # article.count(" ") == substring_rule.count(" ")
    if article.count(" ") != substring_rule.count(" "):
        return matches_rule
    else:
        #for substring_rule in article:
        #pattern = r'\b' + re.escape(substring_rule) + r'\w*'
...
## for規則 引號 "" 中的內容有兩個 星號 *, 輸出你的文章中有出現在 SDGs 關鍵字的部分. 
def check_continuous_words(article, substrings_rule):
   pattern = r'\b' + substrings_rule[0] + r'\w*\b\s+' + substrings_rule[1] + r'\w*\b'
...
def check_continuous_words_rule(article, substrings_rule):
   pattern = r'\b' + substrings_rule[0] + r'\w*\b\s+' + substrings_rule[1] + r'\w*\b'
   #matches = re.search(pattern, article, flags=re.IGNORECASE)
   matches_article = re.findall(pattern, article, flags=re.IGNORECASE)
   matches_rule = []
   if matches_article:
       #print(f"The two continuous words '{match.group(0)}' are present in the article.")
       #matches_rule = [substrings_rule[0], substrings_rule[1]]
       matches_rule = [substrings_rule[0]+'*'+' '+substrings_rule[1]+'*']
   #else:
       #print("The two continuous words are not found in the article.")
   return matches_rule
...
def replace_quoted_content(string):
   ...
   for content in quoted_content:
       if "*" not in content:
           replacement = f'check_strings_0(article, "{content}")'
       elif content.count("*") == 1:
           content_without_asterisk = content.replace("*", "")
           replacement = f'check_strings_1(article, "{content_without_asterisk}")'
       else:
           substrings = [substring.replace("*", "") for substring in content.split() if "*" in substring]
...
##尋找你的文章中有出現在 SDGs 關鍵字集合 key_terms之中的清單key_terms_cited:
def find_key_terms_cited_article(article, key_terms):
   key_terms_cited_article = [] 
   ...
   for content in key_terms:
       if "*" not in content:
           ...
           key_terms_cited_article  = key_terms_cited_article + check_strings_0(article, content)
           #key_terms_cited.append(matches.group(0))
       elif content.count("*") == 1:
           content_without_asterisk = content.replace("*", "")
           #matches = check_strings(article, content_without_asterisk)
           key_terms_cited_article  = key_terms_cited_article + check_strings_1(article, content_without_asterisk)
           #key_terms_cited.append(matches.group(0))
       else:
           substrings = [substring.replace("*", "") for substring in content.split() if "*" in substring]
           key_terms_cited_article  = key_terms_cited_article + check_continuous_words(article, [substrings[0], substrings[1]])
           #matches = check_continuous_words(article, [substrings[0], substrings[1]])
           #key_terms_cited.append(matches.group(0))
   
   key_terms_cited_article = list(set(key_terms_cited_article))        
   return key_terms_cited_article


def find_key_terms_cited_rule(article, key_terms):
   key_terms_cited_rule = [] 
   #key_terms = ["promot* regime*"]
   #key_terms = ["promot*"]
   #content = "promot* regime*"
   for content in key_terms:
       if "*" not in content:
           #matches = check_strings(article, content)
           key_terms_cited_rule  = key_terms_cited_rule + check_strings_0_rule(article, content)
           #key_terms_cited.append(matches.group(0))
       elif content.count("*") == 1:
           content_without_asterisk = content.replace("*", "")
           #matches = check_strings(article, content_without_asterisk)
           key_terms_cited_rule  = key_terms_cited_rule + check_strings_1_rule(article, content_without_asterisk)
           #key_terms_cited.append(matches.group(0))
       else:
           substrings = [substring.replace("*", "") for substring in content.split() if "*" in substring]
           key_terms_cited_rule  = key_terms_cited_rule + check_continuous_words_rule(article, [substrings[0], substrings[1]])
           #matches = check_continuous_words(article, [substrings[0], substrings[1]])
           #key_terms_cited.append(matches.group(0))
   
   #key_terms_cited_rule = list(set(key_terms_cited_rule))        
   return key_terms_cited_rule



# key_terms = ["promot* regime*"]
# article = "promotsss regimesdew"
# find_key_terms_cited_rule(article, key_terms)
...
# string_rule = 'I am happy now and hope to continue.'
# center_quote = 'now'
# string_rule.replace(center_quote, f"<span style='color: red;'>{center_quote}</span>")

#def highlight_center_quote(string_rule, center_quote,DisplayRangeLevel):
def highlight_center_quote(string_rule, center_quote, color):    
    # 以紅色標示中心引號內容並擴展5個括弧內容
    #pattern = r'(\(.*?\))'
    #matches = re.findall(pattern, string_rule, flags=re.IGNORECASE)
    #center_quote = matches[0]
    #highlighted_string = string.replace(center_quote, '\033[91m{}\033[0m'.format(center_quote))
    #highlighted_string = string.replace(center_quote, center_quote.upper())
    #highlighted_string = string_rule.replace(center_quote, center_quote)
    #highlighted_string = highlighted_string.replace(center_quote, f"<span style='color: red;'>{center_quote}</span>")
    #highlighted_string = string_rule.replace(center_quote, f"<span style='color: red;'>{center_quote}</span>")
    center_quote = "\""+center_quote+"\""   ## 完整的 "關鍵字" 加上引號""的字串才會在以下replace()中被尋找並替換掉.
    highlighted_string = string_rule.replace(center_quote, f"<span style='color: {color};'>{center_quote}</span>")
    # for match in matches:
    #     #if center_quote.lower() == find_quoted_content(match)[0].lower():
    #     if center_quote.lower() in find_quoted_content(match):
    #         index = matches.index(match)
    #         start_index = max(0, index-DisplayRangeLevel)
    #         end_index = min(len(matches), index+DisplayRangeLevel+1)
    #         for i in range(start_index, end_index):
    #             #highlighted_string = highlighted_string.replace(matches[i], '\033[91m{}\033[0m'.format(matches[i]))
    #             #highlighted_string = highlighted_string.replace(matches[i], matches[i].upper())
    #             #matches[i]=[st.markdown(:red[j])) for j in matches[i]]
    #             #matches[i] = ''.join(matches[i])
    #             #matches[i]=st.markdown(:red[matches[i]])
    #             #highlighted_string = highlighted_string.replace(matches[i], matches[i].upper())
    #             highlighted_string = highlighted_string.replace(matches[i], f"<span style='color: red;'>{matches[i]}</span>", 1)
    #             #highlighted_string = highlighted_string.replace(matches[i], format_text(matches[i].upper(), style="italic"))
    #             #highlighted_string = string.replace(matches[i], matches[i].upper())
...
#     combined_string = '\n\n\--------------------------\n'.join(substrings)
#     #separator = '\n--------------------------\n'
#     #final_string = separator.join([separator.join(substring.splitlines()) for substring in combined_string.split(separator)])
#     return combined_string

# # 測試程式碼
# substrings = ['Hello', 'World', 'Python', 'Programming']
# result = combine_substrings(substrings)
# print(result)
...
SDGs_choice = st.text_input('**輸入您要查詢的 SDGS 項目編號(整數 1-17, 目前只有 17)**', '17')
...
if SDGs_choice == '17':
    ...
    '''  
    for i in find_quoted_content(cond2):
        key_terms.append(i)        
    ...
    '''  
    for i in find_quoted_content(cond3):
        key_terms.append(i)        
    ...
    # for i in find_quoted_content(cond4):
    #     key_terms.append(i)        
    ...
    # '''  
    # ## 1407th line: W/4 -> and
    # for i in find_quoted_content(cond5):
    #     key_terms.append(i)        
    ...
    '''  
    #print(cond6)
    for i in find_quoted_content(cond6):
        key_terms.append(i)        
    ...
    '''  
    for i in find_quoted_content(cond7):
        key_terms.append(i)        
    ...
    # )\
    # '''  
    # for i in find_quoted_content(cond8):
    #     key_terms.append(i)        
    ...
    '''  
    for i in find_quoted_content(cond9):
        key_terms.append(i)        
    ...
    '''  
    for i in find_quoted_content(cond10):
        key_terms.append(i)        
    ...
    # )\
    # '''  
    # for i in find_quoted_content(cond11):
    #     key_terms.append(i)        
    ...
    '''  
    for i in find_quoted_content(cond12):
        key_terms.append(i)        
    ...
    '''  
    for i in find_quoted_content(cond13):
        key_terms.append(i)        
    ...
    '''  
    for i in find_quoted_content(cond14):
        key_terms.append(i)        
    ...
    # ( TITLE-ABS-KEY ( "SDG" OR "SDGs" OR "sustainable develop*" ) OR TITLE-ABS-KEY ( "poverty" OR "eradicate" OR "reduc*" OR "decreas*" OR "annihilat*" OR "fight*" OR "anti" OR "against" ) )
    # AND
    
    # ( TITLE-ABS-KEY ( "policy space" ) OR TITLE-ABS-KEY ( "recogniz*" OR "contest*" OR "critici*" OR "challeng*" OR "respect" OR "unquestion*" OR "undisput*" OR "accept" OR "accepted" OR "accepting" OR "state action*" OR "state decision*" OR "state leadership" OR "national leadership" OR "government's leadership" OR "national leadership" OR "state intervention" OR "governmental intervention*" OR "governmental decision*" OR "government's action*" OR "governmental action*" OR "country's autonomy" OR "government autonomy" OR "country's strategic approach*" OR "government's strategic approach*" ) )
    # )\
    # '''  
    # for i in find_quoted_content(cond15):
    #     key_terms.append(i)        

    # #key_terms = key_terms+find_quoted_content(cond15)
    # #len(key_terms)  # 295
    # #print(key_terms)
    ...
    # cond15_1 = cond15.replace('TITLE-ABS-KEY', "")
    # #print(cond3_1)
    # # cond1_2 = cond1_1.replace('*"', '"')
    # # print(cond1_2)
    # #pattern = r'\*'
    ...
    # cond15_2 = cond15_1.lower()
    # #print(cond3_2)
    
    
    # ## {}改成""
    # pattern = r'[{}]'
    # cond15_3 = re.sub(pattern, '"', cond15_2)
    # #print(cond3_3)
    #cond15_3_or = cond15_3+' OR'
    #print(cond19_3_or)
    
    # cond15_4 = replace_quoted_content(cond15_3)
    # #print(cond3_4)
    # cond15_final = eval(cond15_4)
    


    ### 條件 16: 
    cond16='''\
    ( TITLE-ABS-KEY ( ( "SDG" OR "SDGS" OR "sustainable development goal*" OR "millennium development goal*" OR "2030 Agenda" OR "Agenda2030" OR "Agenda 2030" ) AND ( "global partnership" OR "international cooperation*" OR "international co-operation*" OR "south-south cooperation*" OR "north-south cooperation*" OR "south-south cooperation*" OR "south-south co-operation*" OR "north-south co-operation*" OR "south-south co-operation*" OR "international collab*" OR "south-south collab" OR "north-south collab*" OR "south-north collab*" OR "capacity building" OR "globalization" OR "globalisation" OR "metagovernance" OR "global governance" OR "global scale" OR "collaborat*" OR "partnership*" OR "cooperation*" OR "co-operation*" OR "global development*" OR "development cooperation" OR "development co-operation" OR "international community" ) ) )\
    '''  
    for i in find_quoted_content(cond16):
        key_terms.append(i)        

    #key_terms = key_terms+find_quoted_content(cond16)
    #len(key_terms)  # 295
    #print(key_terms)
    ...
    cond16_1 = cond16.replace('TITLE-ABS-KEY', "")
    #print(cond3_1)
    # cond1_2 = cond1_1.replace('*"', '"')
    # print(cond1_2)
    #pattern = r'\*'
    ...
    cond16_2 = cond16_1.lower()
    #print(cond3_2)
    
    
    ## {}改成""
    pattern = r'[{}]'
    cond16_3 = re.sub(pattern, '"', cond16_2)
    #print(cond3_3)
    cond16_3_or = cond16_3+' OR'
    #print(cond19_3_or)
    
    cond16_4 = replace_quoted_content(cond16_3)
    #print(cond3_4)
    cond16_final = eval(cond16_4)
    


    ### 條件 17: 
    cond17='''\
    ( TITLE-ABS-KEY ( "public-private partnership*" OR "private-public partnership*" OR "public partnership*" OR "civil society partnership*" OR "public-private coop*" OR "PPP coop*" OR "PPP project*" OR "PP project*" OR "PP participation*" OR "public-private participation*" OR "private-public participation*" OR "public-private project*" OR "private-public project*" OR "ppp contract*" OR "ppp project*" OR "business improvement district*" OR "encourag*" OR "foster*" OR "help*" OR "promot*" OR "increas*" OR "augment*" OR "positiv*" OR "negativ*" OR "discourag*" OR "inhib*" OR "reduc*" OR "lead*" OR "led" OR "bring" OR "flourish*" OR "success*" OR "unsuccess*" OR "benefi*" OR "advantag*" OR "disadvantag*" OR "optimiz*" OR "optimis*" OR "challeng*" OR "risk assessment*" OR "performanc*" OR "efficien*" OR "strateg*" OR "involv*" OR "meaningful" OR "opportunit*" OR "impact*" OR "risk allocation*" OR "possibilit*" OR "develop*" OR "risk management*" OR "model" OR "lessons" OR "share risk*" OR "shared risks" ) )\
    '''  
    for i in find_quoted_content(cond17):
        key_terms.append(i)        

    #key_terms = key_terms+find_quoted_content(cond17)
    #len(key_terms)  # 295
    #print(key_terms)
    ...
    cond17_1 = cond17.replace('TITLE-ABS-KEY', "")
    #print(cond3_1)
    # cond1_2 = cond1_1.replace('*"', '"')
    # print(cond1_2)
    #pattern = r'\*'
    ...
    cond17_2 = cond17_1.lower()
    #print(cond3_2)
    
    
    ## {}改成""
    pattern = r'[{}]'
    cond17_3 = re.sub(pattern, '"', cond17_2)
    #print(cond3_3)
    cond17_3_or = cond17_3+' OR'
    #print(cond19_3_or)
    
    cond17_4 = replace_quoted_content(cond17_3)
    #print(cond3_4)
    cond17_final = eval(cond17_4)
    ...
    ( TITLE-ABS-KEY ( "public-private partnership*" OR "private-public partnership*" OR "public partnership*" OR "civil society partnership*" OR "public-private coop*" OR "PPP coop*" OR "PPP project*" OR "PP project*" OR "PP participation*" OR "public-private participation*" OR "private-public participation*" OR "public-private project*" OR "private-public project*" OR "ppp contract*" OR "ppp project*" OR "business improvement district*" OR "encourag*" OR "foster*" OR "help*" OR "promot*" OR "increas*" OR "augment*" OR "positiv*" OR "negativ*" OR "discourag*" OR "inhib*" OR "reduc*" OR "lead*" OR "led" OR "bring" OR "flourish*" OR "success*" OR "unsuccess*" OR "benefi*" OR "advantag*" OR "disadvantag*" OR "optimiz*" OR "optimis*" OR "challeng*" OR "risk assessment*" OR "performanc*" OR "efficien*" OR "strateg*" OR "involv*" OR "meaningful" OR "opportunit*" OR "impact*" OR "risk allocation*" OR "possibilit*" OR "develop*" OR "risk management*" OR "model" OR "lessons" OR "share risk*" OR "shared risks" ) )\
    '''  
    for i in find_quoted_content(cond18):
        key_terms.append(i)        

    #key_terms = key_terms+find_quoted_content(cond18)
    #len(key_terms)  # 295
    #print(key_terms)
    ...
    cond18_1 = cond18.replace('TITLE-ABS-KEY', "")
    #print(cond3_1)
    # cond1_2 = cond1_1.replace('*"', '"')
    # print(cond1_2)
    #pattern = r'\*'
    ...
    cond18_2 = cond18_1.lower()
    #print(cond3_2)
    
    
    ## {}改成""
    pattern = r'[{}]'
    cond18_3 = re.sub(pattern, '"', cond18_2)
    #print(cond3_3)
    cond18_3_or = cond18_3+' OR'
    #print(cond19_3_or)
    
    cond18_4 = replace_quoted_content(cond18_3)
    #print(cond3_4)
    cond18_final = eval(cond18_4)
    ...
    TITLE-ABS-KEY ( "SDG" OR "SDGS" OR "sustainable development goal*" OR "millennium development goal*" OR "2030 Agenda" OR "Agenda2030" OR "Agenda 2030" OR "measurement*" OR "contribution*" OR "assessment*" OR "completeness" OR "progress towards" OR "fulfilness" OR "achievement*" OR "completion" )
    AND
    
    TITLE-ABS-KEY ( "measurement tool*" OR "assessment tool*" OR "monitoring tool*" OR "indicator*" OR "target measurement*" OR "other than GDP" OR "other than gross domestic product" OR "statistical capacit*" OR "tracking progress" OR "greenmetric" )
    )\
    '''  
    for i in find_quoted_content(cond19):
        key_terms.append(i)        

    #key_terms = key_terms+find_quoted_content(cond19)
    #len(key_terms)  # 295
    #print(key_terms)
    ...
    cond19_1 = cond19.replace('TITLE-ABS-KEY', "")
    #print(cond3_1)
    # cond1_2 = cond1_1.replace('*"', '"')
    # print(cond1_2)
    #pattern = r'\*'
    ...
    cond19_2 = cond19_1.lower()
    #print(cond3_2)
    
    
    ## {}改成""
    pattern = r'[{}]'
    cond19_3 = re.sub(pattern, '"', cond19_2)
    #print(cond19_3)
    #cond19_3_or = cond19_3+' or'
    #print(cond19_3_or)
    
    cond19_4 = replace_quoted_content(cond19_3)
    #print(cond3_4)
    cond19_final = eval(cond19_4)
   

    #substrings_rule = [cond1_3, 'OR', cond2_3, 'OR', cond3_3, 'OR', cond6_3, 'OR', cond7_3, 'OR', cond9_3, 'OR', cond10_3, 'OR', cond12_3, 'OR', cond13_3, 'OR', cond14_3, 'OR', cond16_3, 'OR', cond17_3, 'OR', cond18_3, 'OR', cond19_3]
    #substrings_rule = [cond1_3, cond2_3, cond3_3, cond6_3, cond7_3, cond9_3, cond10_3, cond12_3, cond13_3, cond14_3, cond16_3, cond17_3, cond18_3, cond19_3]
    substrings_rule = [cond1_3_or, cond2_3_or, cond3_3_or, cond6_3_or, cond7_3_or, cond9_3_or, cond10_3_or, cond12_3_or, cond13_3_or, cond14_3_or, cond16_3_or, cond17_3_or, cond18_3_or, cond19_3]
    combined_string_rule = combine_substrings(substrings_rule)
    #print(combined_string_rule)
         
    
    
    
    
    if cond1_final or cond2_final or cond3_final or cond6_final or cond7_final or cond9_final or cond10_final or cond12_final or cond13_final or cond14_final or cond16_final or cond17_final or cond18_final or cond19_final:
    ...
    with st.expander(f"**你的論文中屬於 SDGs {SDGs_choice} 的關鍵字如下:**"):
        ...
        # #result = find_words_with_prefix(input_string, input_list)
        key_terms_cited_article = find_key_terms_cited_article(article, key_terms)
        for string in key_terms_cited_article:
            st.write(string)



    with st.expander(f"**SDGs {SDGs_choice} 特定關鍵字相關之收錄規則查詢 (請參考查詢結果紅色標註之關鍵字附近之規則, 收錄規則的大項之間以藍色 'OR' 分隔):**"):
        #string = cond19_3
        #string_rule = cond1_3
        #type(cond1_3)
        #print(cond1_3)
        #modified_string_rule = remove_asterisks(combined_string_rule)
        #print(modified_string)
        
        
        
        #search_text = input("請輸入要搜索的文本：")
        article_keyTerm = st.text_input('**請輸入要搜索的 關鍵字 或 詞彙：**')
        DisplayRangeLevel=st.slider('**選擇一個整數以決定以關鍵詞彙為中心的標註範圍 (關鍵詞彙鄰近之其他詞彙以粉紅色標註)**', 0, 100, 2)
        #matching_rule_quotes = find_matching_quotes(modified_string_rule, article_keyTerm)
        matching_rule_quotes = find_key_terms_cited_rule(article_keyTerm, key_terms)
        #matching_quotes = find_matching_quotes(input_string, search_text)
        #print("符合的引號內容：", matching_quotes)
        
        #key_terms = ["promot* regime*"]
        #article_keyTerm = "promotsss regimesdew"
        #matching_rule_quotes = find_key_terms_cited_rule(article_keyTerm, key_terms)   ## ['promot* regime*']  ##['promot', 'regime']
        #type(matching_rule_quotes)  
        
        highlighted_string = combined_string_rule
        if matching_rule_quotes:
            st.write("**你輸入的詞彙有在收錄規則中, 請參考紅色標註關鍵字附近之收錄規則 !**")
            #st.write(f"**收錄規則中的關鍵詞彙如下: {matching_rule_quotes}**")
            for center_quote in matching_rule_quotes:
            #center_quote = input("請選擇中心引號內容：")
                #center_quote = i
                #index = key_terms.index(center_quote)
                multiple_positions = [index for index, value in enumerate(key_terms) if value == center_quote]
                for index in multiple_positions:
                    #highlighted_string = highlight_center_quote(modified_string_rule, center_quote, DisplayRangeLevel)
                    #highlighted_string = highlight_center_quote(modified_string_rule, center_quote)
                    start_index = max(0, index-DisplayRangeLevel)
                    end_index = min(len(key_terms), index+DisplayRangeLevel+1)
                    for i in range(start_index, end_index):
                        if i==index:
                            #highlighted_string = highlight_center_quote(highlighted_string, center_quote)
                            highlighted_string = highlight_center_quote(highlighted_string, key_terms[i].lower(),'red')
                            #st.write("以大寫標示中心引號內容並擴展2個括弧內容：", highlighted_string)
                        else:
                            highlighted_string = highlight_center_quote(highlighted_string, key_terms[i].lower(),'pink')  
        else:
            st.write("**你輸入的詞彙沒有在收錄規則中 !**")
            #st.markdown(highlighted_string, unsafe_allow_html=True)
        center_quote_OR = 'OR'
        highlighted_string = highlighted_string.replace(center_quote_OR, f"<span style='color: blue;'>{center_quote_OR}</span>")
        #st.write(highlighted_string)
        st.markdown(highlighted_string, unsafe_allow_html=True)
    ...
    with st.expander(f"**SDGs {SDGs_choice} 重要關鍵字列表, 添加這些關鍵字於 title, abstract, keywords 將有助於論文被收錄於 SDGs {SDGs_choice}. 完整收錄條件如下連結 https://tinyurl.com/26xxakca.**"):
        ...
        key_terms_noRepeated = list(set(key_terms))
        st.write(key_terms_noRepeated)
    ...
    st.write("**此項次 SDGs 尚未建置 !**")
    
    
st.markdown("---")
st.markdown("製作者：羅主斌")
st.markdown("版權所有 © 2023")
