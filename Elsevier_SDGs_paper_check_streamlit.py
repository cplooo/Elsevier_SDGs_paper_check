...
if SDGs_choice == '17':
    ...
    with st.expander(f"**SDGs {SDGs_choice} 特定關鍵字相關之收錄規則查詢 (請參考查詢結果紅色標註之關鍵字附近之規則, 收錄規則的大項之間以藍色 'OR' 分隔):**"):
        ...
        DisplayRangeLevel=st.slider('**選擇一個整數以決定以關鍵詞彙為中心的標註範圍 (關鍵詞彙鄰近之其他詞彙以粉紅色標註, 國名無法標註顏色)**', 0, 100, 2)
                ...
                for index in multiple_positions:
                    ...
                    for i in range(start_index, end_index):
                        if i==index:
                            ...
                            highlighted_string = highlight_center_quote(highlighted_string, key_terms[i],'red')
                            #st.write("以大寫標示中心引號內容並擴展2個括弧內容：", highlighted_string)
                        else:
                            highlighted_string = highlight_center_quote(highlighted_string, key_terms[i],'pink')  
        else:
